...
- [x] **4.2** Дедуп близких фактов — **сделано** (детект + ассист, без авто-merge: ложное слияние фактов в «мозге» дорого). pgvector self-join (`findDuplicatePairs`, cosine-distance ≤ порог, исключая self/один item/забытые) → `MemoryDeduplicationService` выбирает канон (выше confidence, старше на ничью). Выдача: `GET /api/memory-units/duplicates?maxDistance=&limit=` + бот `/duplicates` (нумерованный список, лишнее убирается готовым `/forget <n>`). Тесты: repo-интеграция (пары находятся через items, забытые исключаются). Авто-консолидация — отложена.
...
- [x] **6.2 Полный вариант (REST + хранилище)** — **сделано.** Решение по хранилищу: **`bytea` в отдельной таблице `stored_media`** (миграция 016, ключ = `inbox_items.id`) — просто, транзакционно, не грузит обычные запросы (блоб в своей таблице/entity). `MediaStorageService` (store/get), `MediaController`: `POST /api/inbox-items/image` (multipart → vision → `create` с тегом `image` → сохранить байты) + `GET /api/inbox-items/{id}/image` (отдача с content-type). Тест: round-trip bytea (`MediaStorageServiceTest`). ⚠️ окупается при не-Telegram клиенте (которого пока нет) — как и REST auth.
- [⏸] **6.3 (отложено по решению):** CLIP-эмбеддинги изображений для визуального «найди похожую картинку». Требует внешней инфры (CLIP недоступен через используемый OpenAI API — нужен локальный сервис/сторонний провайдер) + второе векторное пространство (колонка/индекс/путь эмбеддинга/запрос). Поиск картинок по содержимому уже закрыт vision-описанием → маржинальная ценность низкая. Вернуться при реальной потребности в визуальном сходстве.
...
- [x] **8.3 Разбор ссылок** — **сделано.** В `process()`: если есть ссылка → `LinkContentService` (Jsoup fetch+strip HTML, таймаут 10с, кап 8000 симв.) → `AiArticleSummaryService` (OpenAI → JSON {title, summary}) → добавляется `ARTICLE`-юнит (summary + excerpt + теги `article`/домен), эмбеддится вместе со всеми → статья ищется по содержимому. Best-effort: сбой fetch/summary/openai-off → юнит не добавляется, ссылка-тег остаётся. Зависимость `org.jsoup:jsoup`. Тест `AiArticleSummaryServiceTest`.

## Фаза 9 — Диалоговый ассистент (от консоли к разговору)

**Цель:** убрать «командный» характер — минимум слэш-команд и номеров, максимум естественного языка и тапов; чистая обратная связь без мусора.

### 9.1 UX-полировка ответов и захвата — **сделано**
- [x] **Реакции вместо ack-текста.** Убраны «Сохранил…/Тип/Теги», «Сохранил картинку/голосовое». Теперь: обработка → 👀 (ставится при захвате), успех → 👍, сбой → 👎 (через `InboxItemProcessedEvent` + `messageId`/`success`; `TelegramConnectionNotifier` ставит финальную реакцию). Telegram разрешает только фиксированный набор реакций — 👀/👍/👎 вместо ⏳/✅/❌.
- [x] **Источники только по запросу.** По умолчанию ответ — просто текст, без блока источников. Если в запросе есть «источник» — сырьё исходной записи + дата, без типа/тегов.
- [x] **Баг: дедуп источников по сообщению.** Источники схлопываются по `inbox_item_id` (одно сообщение = один источник) — чинит кейс «часы Casio» (мама подарила + потерял). Тесты `answerHasNoSourcesByDefault`/`answerShowsRawSourceWhenRequested`/`dedupesSourcesFromSameMessage`.

### 9.2 Действия естественным языком + резолв цели по смыслу — **сделано**
- [x] `TelegramActionDetector` (AI, гейт по глаголам-префиксам → LLM только на похожих на команду сообщениях; degrade при openai off) распознаёт `COMPLETE/SNOOZE/FORGET/EDIT` + `target` (описание цели) + `param` (срок снуза / новый текст). «напомни …» → NONE (это capture). Резолв цели: задачи — по пересечению токенов с открытыми задачами; записи (forget/edit) — через `inboxItemSearchService.search(target)` с дедупом по сообщению. Тест `TelegramActionDetectorTest`.

### 9.3 Подтверждение и уточнение (мини-диалог) — **сделано**
- [x] Per-chat `pendingActions`: 1 цель → COMPLETE/SNOOZE/EDIT сразу, FORGET с подтверждением «да/нет»; несколько → нумерованный список «какую?» → ответ номером; «нет/отмена» → отмена; не-ответ → pending сбрасывается, сообщение трактуется заново. Команды `/done`/`/forget` остаются точным запасным путём. Тесты: `completesTaskByNaturalLanguage`, `forgetsMemoryByNaturalLanguageAfterConfirmation`.

### 9.4 Inline-кнопки (callback_query)
- [ ] Под списками/уточнениями — тапаемые кнопки `[✅][⏰ 1ч][⏰ Завтра][✏️][🗑]`. Обработка `callback_query`. Снимает печать и неоднозначность.

### 9.5 «Напомни …» как первоклассное действие
- [ ] `напомни завтра в 18 …` → сразу создаёт напоминание (разбор времени уже есть в extraction).

### 9.6 Многоходовый Q&A
- [ ] Контекст последнего ответа per-chat: «а когда это было?», «а сколько стоило?» без повтора вопроса.

### 9.7 Дружелюбный тон, скрытые команды — **сделано**
- [x] `/help` переписан в естественных формулировках с примерами (сохрани/найди/задачи/правка/«источник»); слэш-команды показаны как необязательные ярлыки. Тест проверяет ключевые фразы (не точную строку).

**Принцип:** забывание/правку всегда подтверждаем; точные команды (`/done`, `/forget`) остаются запасным путём, просто не на виду.
