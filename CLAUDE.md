...
2. `AiClassificationService` (OpenAI chat completion, JSON mode) classifies the raw text into title/summary/`InboxItemType`/priority/actionable/tags. Failure throws `AiProcessingUnavailableException` and the item is **not** saved — there's no degraded "save without AI" path.
3. The raw `InboxItem` is persisted with classification results applied.
