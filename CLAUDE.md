...
Recognized intents: HELP, RECENT, TODAY, SEARCH, CAPTURE/UNKNOWN (falls through to saving the message via `InboxItemService.captureAsync` — persists the raw item and acks instantly with "Сохранил, разберу позже", AI enrichment runs on a background thread). `/type <TYPE>` retroactively retypes the chat's last item; `/embeddings backfill` triggers `InboxItemEmbeddingBackfillService`. `/tasks` lists open TASK/REMINDER units for the chat (remembering the ordered list per chat), `/done <n>` / `/snooze <n> <30m|2h|1d>` act on that list (`MemoryTaskService`). `/forget <n>` / `/edit <n> <text>` act on the **last shown SEARCH list** per chat (the bot remembers exactly the numbered list it rendered — cited sources when an answer was shown, else all hits) via `MemoryEditService`: forget is a reversible soft-delete (`memory_units.forgotten_at`), edit rewrites the unit's title+summary and re-embeds (no re-extraction). Due TASK/REMINDER units (`MemoryUnit.dueAt`, populated by extraction from the note's date/time) are delivered to their originating chat by `TelegramReminderScheduler` (`@Scheduled`, only when the bot is enabled), marking `reminded_at` after a successful send. SEARCH responses combine `InboxItemSearchService` results with an optional `MemoryAnswerService` answer via `TelegramSearchResponseFormatter`, then re-send the actual photos behind any image-backed hits (up to 3) via `sendPhoto(file_id)`.

**Images (MVP, Telegram-only, no binary storage):** a photo message is intercepted in `handleUpdate` (before the text-empty rejection) and handed to `TelegramImageIngestionService.ingest` (`@Async`, gated by `telegram.bot.enabled`); the bot acks instantly with "Сохранил картинку, разберу позже". Ingestion downloads the largest `PhotoSize` (`TelegramApiClient.getFile` + `downloadFile`), runs it through `AiVisionService.describe` → `OpenAiClient.describeImage` (vision on the same `openai.model`, reusing the 1.3 retry layer) to get a description + OCR text, and saves it via `InboxItemService.create(request, fileId, "image/jpeg")` with an `image` tag. The vision text becomes `rawText` (caption + description, or placeholder "Изображение" if both empty, so `raw_text NOT NULL` holds), so the image is classified/extracted/embedded and **searched/answered by the existing text pipeline unchanged**. The Telegram `file_id` is stored on `inbox_items.image_file_id` (+ `media_type`, migration `013`) and surfaced via `MemoryUnitResponse.imageFileId` so the photo can be re-sent on retrieval. When OpenAI is disabled, ingestion degrades to caption-only/placeholder text but still captures the `file_id`.
