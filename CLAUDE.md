...
3a. If the note contains a link (Feature 8.3), `LinkContentService` (Jsoup) fetches + strips the page and `AiArticleSummaryService` summarizes it into an extra `ARTICLE` `MemoryUnit` (best-effort: any failure leaves just the link tag), so saved links become searchable by their actual content. On successful processing an `InboxItemProcessedEvent` is published; `TelegramConnectionNotifier` (Feature 8.1, bot-gated, AFTER_COMMIT) surfaces semantically related past memories ("🔗 Похоже на прошлое"). A scheduled `TelegramDigestScheduler` (Feature 8.2) sends a proactive morning digest (open tasks + recent captures) built by `MemoryDigestService`.
